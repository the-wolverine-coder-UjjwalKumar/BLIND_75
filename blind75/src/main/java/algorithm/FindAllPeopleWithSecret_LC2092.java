...
public class FindAllPeopleWithSecret_LC2092 {
...
//        int firstPerson = 1;
    ...
    private static List<Integer> findAllPeople(int n, int[][] meetings, int firstPerson) {
        ...
        for (int row = 0; row < meetings.length; row++) {
            int p1 = meetings[row][0];
            int p2 = meetings[row][1];
            int t = meetings[row][2];

            if (p1 == firstPerson || p2 == firstPerson || map.containsKey(p1) || map.containsKey(p2)) {

                int p1OldMeetTime = map.getOrDefault(p1, -1);
                int p2OldMeetTime = map.getOrDefault(p2, -1);

                if (p1OldMeetTime==-1 || t > p1OldMeetTime){
                    map.put(p1, t);
                    ans.add(p1);
                }

                if (p2OldMeetTime == -1 || t > p2OldMeetTime) {
                    map.put(p2, t);
                    ans.add(p2);
                }
